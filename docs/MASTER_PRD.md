...
'자기관리(Habit Tracker)'와 '대항해시대풍 무역 시뮬레이션(Trade Sim)'을 결합한 안드로이드 앱. 사용자의 실제 성취(할 일 달성)가 게임 내 배의 추진력이 되고, 전략적인 항해와 무역을 통해 정착지를 발전시킵니다. 스타듀밸리와 같은 오픈 엔딩 방식의 스토리와 다양한 NPC와의 상호작용을 통해 깊이 있는 모험을 제공합니다.
...
### 3.5. Narrative & Interaction (스토리 및 상호작용)
- **Open-Ended Story (오픈 엔딩):**
    - **Concept:** 정해진 엔딩이 없는 '스타듀밸리' 식의 생활/모험 시뮬레이션.
    - **Progression:** 플레이어의 탐험과 성장에 따라 자연스럽게 해금되는 스토리와 미스터리.
- **NPC System:**
    - **Ship Crew (선원):** 배에 탑승하여 함께 항해. 습관 달성도(Streak)에 따라 사기가 변화하며, 특정 능력치(속도, 거래 스킬 등) 버프 제공.
    - **Port Locals (항구 주민):** 각 항구의 특색을 반영한 거주민. 무역 정보 제공, 퀘스트 부여, 또는 지역의 전설을 들려줌.
    - **Settlers (정착민):** 플레이어가 건설한 정착지에 이주해 온 주민. 세금 징수, 자원 생산을 담당하며 플레이어와 상호작용.
- **Relationship (호감도):** 대화, 선물, 퀘스트 해결을 통해 NPC와 친밀도를 쌓고 숨겨진 스토리나 보상을 획득.

### 3.6. World & Events (세계 확장 및 이벤트)
- **Expanded World (확장된 세계):**
    - **Scale:** 기존보다 훨씬 넓어진 대양과 다수의 대륙/섬.
    - **Biomes:** 열대, 빙하, 폭풍우 지대 등 다양한 기후와 환경이 존재하는 해역.
- **Dynamic Event System:**
    - **At Sea (항해 이벤트):** 폭풍우 조우, 조난선 구조, 해적 출몰, 돌고래 떼 발견, 신비한 섬 발견 등 무작위 인카운터.
    - **Seasonal/Scheduled (기간 이벤트):** 계절에 따른 축제, 특정 기간에만 열리는 희귀 교역로, 전설의 물고기 출몰 등.
