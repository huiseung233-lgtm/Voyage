...
- [x] **홈 달력 날짜 미표시**: 양피지 달력에 점수가 나타나지 않고 배치가 어긋나는 문제. (Canvas 격자 도입, 양피지 자동 연장, 점수 위치 최적화로 해결 완료).
- [ ] **점수 초기화 버그**: 아침 점호(confirmDailyBriefing) 시 오늘 날짜의 DailyLog를 0점으로 무조건 덮어쓰는 문제. (기존 데이터 존재 여부 확인 로직 필요).
