...
- [ ] 알림 뜰때 필요한 시간에만 알람이 뜨게 하거나, 시스템바에만 뜨게 하거나 선택할수 있는 선택권 주기.

## 🐛 발견된 버그 (Fix Me!)
- [x] **정박 시스템 버그**: 배가 정박 상태로 전환되지 않는 문제. (해결 완료)
- [x] **설정창 크래시**: 알람 주기 설정 시 숫자를 모두 지우면 앱이 강제 종료되는 문제.
- [x] **상단 상태바 가독성**: 시스템 상태바 색상이 너무 밝아 정보가 보이지 않는 문제. (`HomeScreen.kt`의 Deprecated된 `statusBarColor` 코드를 `WindowInsetsController`로 교체 필요).
...
- [x] **로그북 공용화 & 정박 프로세스**: 게임 화면에서 로그북 작성 시 화면 종료 없이 정박하는 기능 구현 완료. 데이터 연동 및 플래그 관리 로직 최적화.
- [x] **점수 입력창 색상 개선**: 입력창의 색상 대비가 낮아 가독성이 떨어지는 문제.
- [x] **토스트 메시지 추가**: 점수 기록 추가/삭제 시 "추가되었습니다", "삭제되었습니다" 알림 띄우기.
- [x] **AlarmReceiver 부팅 로직 수정**: 폰 재부팅 시 `BOOT_COMPLETED` 신호를 받으면 알림을 띄우지 않고 알람만 재등록하도록 수정 필요 (현재는 무조건 알림이 뜸). `RECEIVE_BOOT_COMPLETED` 권한도 추가 필요.
- [x] **알람 안울리는 버그** 세팅 창의 정기 주기 알림 설정을 해도 알람이 안울림.
- [x] **알람 안뜰 시간을 정하는 기능 추가**