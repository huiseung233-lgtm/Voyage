...
## 📝 Recent Updates
- **[Feature]** 애니메이션 시스템 구축: `ui/animation` 패키지 신설 및 `SailingScene` 엔진 구현. (아이소메트릭 항해 뷰, 동적 파도/날씨/포말 효과)
- **[UI]** 메인/목표 화면 개편:
    - 메인 탭: 실시간 항해 애니메이션 배경 적용 및 정박/항해 상태별 연출 차별화.
    - 목표 탭: 상단 배너에 실시간 미니맵 적용 및 전체 지도 다이얼로그 최적화.
- **[Refactor]** 코드 구조 및 성능 개선:
    - 렌더러(Renderer) 인터페이스 도입으로 그래픽 확장성 확보.
    - `WorldMapView` 리팩토링 및 `GoalsScreen` 내 불필요한 배경 애니메이션 제거.
